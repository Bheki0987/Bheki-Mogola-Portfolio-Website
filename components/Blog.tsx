...
import { Calendar, X, ChevronRight, ArrowRight } from 'lucide-react';
...
const Blog: React.FC = () => {
    ...
    const formatContent = (content: string) => {
        ...
        // Find the first non-empty line to determine the baseline indentation
        const firstLine = lines.find(line => line.trim().length > 0);

        if (!firstLine) return content.trim();

        // Capture the leading whitespace of the first line
        const indentMatch = firstLine.match(/^\s*/);
        const indent = indentMatch ? indentMatch[0] : '';

        if (!indent) return content.trim();

        // Strip that specific indentation from all lines
        return lines.map(line => {
            if (line.startsWith(indent)) {
                return line.slice(indent.length);
            }
            // If deeper indentation, strip the baseline amount
            // If shallower (shouldn't happen for valid code blocks if based on 1st line), trim start
            return line.trimStart();
        }).join('\n').trim();
    };

    return (
        <section id="blog" className="py-24 relative bg-black border-t border-neutral-900">
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
                    ...
                    >
                        <h2 className="text-3xl md:text-4xl font-bold mb-2 text-white">Insights & <span className="text-neutral-500">Updates</span></h2>
                    ...
                    >
                        <Link
                            to="/blog"
                            className="inline-flex items-center gap-2 text-white border-b border-white pb-1 hover:text-blue-400 hover:border-blue-400 transition-colors"
                        >
                        ...
                        >
                            ...
                            <div className="p-8 md:p-12">
                                ...
                                <div className="prose prose-invert prose-neutral max-w-none text-neutral-300">
                                    <div className="whitespace-pre-wrap font-sans text-neutral-300 leading-relaxed text-lg">
